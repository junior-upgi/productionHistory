...
@section('content')
<script src="{{ url('/js/duty/list.js') }}"></script>
<div class="row">
    <div class="col-md-12">
        <h2>生產排程清單</h2>
        <div class="pull-left">
            <a class="btn btn-default" href="{{ url('/Duty/DutyList') }}">返回值班表清單</a>
            <button class="btn btn-primary" onclick="testModelDuty()">填寫試模值班表</button>
        </div>
        <form class="form-inline pull-right" action="{{ url('/Duty/ScheduleList') }}" role="form">
            <div class="row form-group">
                <div class="col-md-3">
                    <input type="text" name="prd_no" class="form-control" placeholder="請輸入瓶號" value="{{ (string) $prd_no }}">
                </div>
            </div>
            <div class="row form-group">
                <div class="col-md-3">
...
<div class="row">
    <div class="col-md-12">
        ...
        <p>
            {{ $list->setPath('/Duty/ScheduleList?prd_no=' . $prd_no . '&machno=' . $machno) }}
        </p>
    </div>
</div>
@include('duty.add')
