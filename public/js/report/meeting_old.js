...
function setTask(list) {
    ...
    for (i=0; i<list.length; i++) {
        var id = list[i]['id'];
        var name = list[i]['SALM_NAME'];
        var des = list[i]['description'];
        var deadline = list[i]['deadline'];
        $('#taskDetail').append(
            '<tr>' + 
                '<td style="width: 50px;"><button type="button" class="btn btn-danger btn-sm">刪除</button></td>' + 
                '<td style="width: 100px;"><span>' + name + '</span></td>' + 
                '<td><span>' + des + '</span></td>' + 
                '<td style="width: 120px;"><span>' + deadline.substr(0, 10) + '</span></td>' + 
            '</tr>'
        );
    }
}
